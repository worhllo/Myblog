Gmeek 默认的分页只有 `上一页` 和 `下一页`，翻页的时候就感觉很无力，没有那种掌控全局的感觉，所以搞一个插件插入数字分页条吧。
## 开搞！！！！！！！！！！！
...
> **Note**
> - 通过解析 `rss.xml` 来获取文章总数
> - 除以每页的文章数，`Math.ceil()` 获得总页数
> - 获取当前 url，并匹配得到当前页数
> - 创建数字分页条。小于等于十页完整显示；大于十页时保留首尾各三页以及当前页和与当前页相邻的两页，省略处用省略号表示
> - 为当前页的页码设置样式
...
参考链接：[给博客添加数字分页条](https://blog.154451.xyz/post/gei-bo-ke-tian-jia-shu-zi-fen-ye-tiao.html)
